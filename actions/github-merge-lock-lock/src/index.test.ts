...
describe('github-merge-lock-lock action', () => {
  ...
  it('sets outputs when lock succeeds', async () => {
    ...
    rulesetMocks.lockBranchWithRuleset.mockResolvedValue(true)

    await run(ctx, deps)

    expect(rulesetMocks.lockBranchWithRuleset).toHaveBeenCalledWith({
      owner: 'owner',
      repo: 'repo',
      branch: 'main',
  ...
  it('logs when already locked', async () => {
    ...
    rulesetMocks.lockBranchWithRuleset.mockResolvedValue(false)
