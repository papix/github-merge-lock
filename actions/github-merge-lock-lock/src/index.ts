...
import { lockBranchWithRuleset, resolveRulesetName } from '#core/rulesets/client.js'
...
export async function run(ctx: RunContext = createDefaultContext(), deps?: RunDeps): Promise<void> {
  ...
  try {
    ...
    const changed = await lockBranchWithRuleset({ owner, repo, branch, rulesetName })
    ...
    if (changed) {
      resolvedDeps.core.info(`Locked branch "${branch}" in ${owner}/${repo} (ruleset: ${resolvedName})`)
    } else {
      resolvedDeps.core.info(`Branch "${branch}" is already locked in ${owner}/${repo} (ruleset: ${resolvedName})`)
    }
  } catch (error) {
