...
import { getRulesetStatus, lockBranchWithRuleset, resolveRulesetName } from '#core/rulesets/client.js'
...
const handleDryRun = async (options: CommandOptions) => {
  ...
  console.log(
    `[DRY-RUN] Would lock branch "${options.branch}" in ${options.owner}/${options.repo}${formatRulesetSuffix(options)}`,
  )
  if (status.found) {
...
const handleSuccess = (options: CommandOptions, changed: boolean) => {
  ...
  if (changed) {
    console.log(`Locked branch "${options.branch}" in ${options.owner}/${options.repo}${formatRulesetSuffix(options)}`)
    return
  }

  console.log(
    `Branch "${options.branch}" is already locked in ${options.owner}/${options.repo}${formatRulesetSuffix(options)}`,
  )
}
...
export const lockCommand = new Command('lock')
  .description('Lock a branch to prevent merges')
  .requiredOption('-o, --owner <owner>', 'Repository owner')
  .requiredOption('-r, --repo <repo>', 'Repository name')
  .requiredOption('-b, --branch <branch>', 'Branch name to lock')
  ...
  .action(async (options: CommandOptions) => {
    try {
      ...
      const changed = await lockBranchWithRuleset({
        owner: options.owner,
        repo: options.repo,
        branch: options.branch,
        rulesetName: options.rulesetName,
